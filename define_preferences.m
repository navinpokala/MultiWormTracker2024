...
Prefs = struct('matlab_exec_path','MATLAB.exe -minimize -nosplash  -r',...  % -nodisplay  -nodesktop -minimize -noawt -noFigureWindows path for launching child matlab processess 
    ...
    'DefaultNumWormRange',[20 30], ...             % most experiments have 20 to 30 animals
...
... % Track linkage parameters    
    ...
    'MaxCentroidShift_mm_per_sec', 0.5, ...        % 0.3 used for linking tracks; empirically determined
    'MaxTrackLinkSeconds',10,...                   % 10 max time (sec ... convert to frames below) between the end of a track and the start of another for linkage
...
Prefs.NumCPU = str2num(getenv('NUMBER_OF_PROCESSORS'))-1; % number of CPUs that can be used
if(isempty(Prefs.NumCPU))
    Prefs.NumCPU = feature('numcores')-1;
end
